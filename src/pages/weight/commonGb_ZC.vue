            ...
            <div slot="fristbox" class="editor">
                    <el-form  :model="formModel"  ref="formModel" label-width="80px" class="formStyle">
                        ...
                        <el-row>
                             ...
                             <el-col :span="6">
                                <el-form-item label="用料单位" prop="o_Col29">
                                    <yl-commonselect ref="commonselect" 
                                            style="width:100%"
                                            v-model="formModel.order.o_Col29" 
                                            size="small"
                                            placeholder="请选择用料单位"
                                            :initData="labourConf.dataList"
                                            :defaultProps="labourConf.defaultProps"
                                            @clear="formModel.order.v_Col46=''"
                              ...
                              <el-col :span="6">
                                    <el-form-item label="工号" prop="o_Col30">
                                            <yl-commonselect ref="commonselect" 
                                                style="width:100%"
                                                v-model="formModel.order.o_Col30" 
                                                size="small"
                                                placeholder="请选择工号"
                                                :initData="GHConf.dataList"
                                                @clear="formModel.order.v_Col47=''"
            ...
            <div slot="secondbox" class="flexbox">
                <yl-layout>
                        ...
                        <el-dialog ref="cameraEdit" :title="'拍照信息'" v-model="openCameraEdit" size="normal"  top="35%" 
                                    ...
                                    <cameraEidt  type="zc" :orderModel="selectRows[0]"  v-if="openCameraEdit"></cameraEidt>
                        </el-dialog>
            </div>
        </yl-layout>
</template>
...
export default {
  ...
  computed: {
            isCarNumAutoRead:{
                get(){
                     let carNumRecognite=this.gbGlobConf.commonConf.zcConf.carNumRecognite;
                     if(carNumRecognite){
            ...
            isCameraEditShow:{
                get(){
                     let speedCamera=this.gbGlobConf.commonConf.zcConf.speedCamera;
                     if(speedCamera){
            ...
            computedjingWeight2:{ //主单位净重
                get(){
                        let jz=0;
                        if(this.formModel.order.b_Col33){
                            jz=new Decimal(this.formModel.order.d_Col41);
                        }else{
                            jz=new Decimal(Math.abs(new Decimal(this.formModel.order.d_Col36).sub(this.formModel.order.d_Col37))).sub(this.formModel.order.d_Col40);
                        }
                        if(this.formModel.order.b_Col26){
                           jz=new Decimal(0).minus(jz);
                        }
                        if(this.isComputations){
                            //乘法
                             if(this.formModel.order.d_Col38>0){
                                    return new Decimal(jz).mul(this.formModel.order.d_Col38).toNumber().getToFixed(this.gbGlobConf.commonConf.baseConf.gbFloatLength.jzl);
                             }else{
                                    return new Decimal(jz).toNumber().getToFixed(this.gbGlobConf.commonConf.baseConf.gbFloatLength.jzl);
                             }
                        }else{
                            //除法
                            if(this.formModel.order.d_Col38>0){
                                return new Decimal(jz).div(this.formModel.order.d_Col38).toNumber().getToFixed(this.gbGlobConf.commonConf.baseConf.gbFloatLength.jzl);
                            }else{
                                return new Decimal(jz).getToFixed(this.gbGlobConf.commonConf.baseConf.gbFloatLength.jzl);
                            }
                        }
                }
             },
                    ...
                    get () {
                        return {
                            ...
                            columns: [
                                ...
                                {attr: { prop: 'v_Col46', label: '用料单位名称', width:120, } },
                                {attr: { prop: 'v_Col47', label: '工号', width:100,  } }, 
   ...
   methods:{
       ...
       _openCameraEdit(){
        ...
        async _printPage(params1,type){
                ...
                if(type==0){
                    //不带扣重
                   printModelID=_this.gbGlobConf.commonConf.zcConf.print.nonkzTemplate.id;
                }else if(type==1){
                    //代扣重
                    printModelID=_this.gbGlobConf.commonConf.zcConf.print.kzTemplate.id;
                }else if(type==2){
                    //电子磅单
                    printModelID=_this.gbGlobConf.commonConf.zcConf.print.imgTemplate.id;
                }
        ...
        _initCompoentData(){
                ...
                this.labourConf=util.deepCopy(this.bakUpData.labourConf);
                this.GHConf=util.deepCopy(this.bakUpData.GHConf);
        ...
        async  _importWight(){
            ...
            _this.formModel.order.v_Col24='直进直出';
            ...
            //保存数据
             ...
             if(result===null){
                ...
                if(_this.gbGlobConf.commonConf!=undefined){
                    if(_this.gbGlobConf.commonConf.zcConf.upload.realTime){
                           //先获去数据
                ...
                if(_this.gbGlobConf.commonConf===undefined){
                    ...
                    if(_this.gbGlobConf.commonConf.zcConf.print.isPrint){
                        //打印提示
                        if(_this.gbGlobConf.commonConf.zcConf.print.isPrintView){
                            //预览
                               _this._printPage(_this.formModel.order,_this.gbGlobConf.commonConf.zcConf.print.type);
        ...
        async _labourChange(node){ 
            ...
            let _this=this;
            if(node[0]!=undefined){
              _this.formModel.order.v_Col46=node[0].labourName;
              _this.formModel.order.i_Col3=node[0].oriBigId;
              _this.formModel.order.v_Col51=node[0].oriStrId;
             }
        },
        _getGHChange(node){
           if(node[0]!=undefined){
                //工号获取事件
                this.formModel.order.v_Col54=node[0].ghFullName;
                this.formModel.order.v_Col47=node[0].ghName;
                this.formModel.order.v_Col53=node[0].ghCode;
                this.formModel.order.i_Col5=node[0].oriBigId;
                this.formModel.order.v_Col52=node[0].oriStrId;
            }
         },
                    ...
                    let  dataModel= await  _this.asyncFunc({
                                                    ...
                                                    data:{
                                                        queryConditionItem:[
                                                                            ...
                                                                            {dataField:"V_Col24",op:'EQ',dataValue:'直进直出'},
